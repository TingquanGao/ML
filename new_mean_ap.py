...
def new_average_precision(recalls, precisions, mode='area'):
...
def new_eval_map(nums,
    ...
    # get test images num
    images_num = len(det_results)
    
    
    #################################################
    # get gt and det bboxes of this class
    cls_dets, cls_gts, cls_gt_ignore = get_cls_results(det_results, gt_bboxes, gt_labels, gt_ignore, 0)
    # calculate tp and fp for each image
    tpfp_func = (tpfp_imagenet if dataset in ['det', 'vid'] else tpfp_default)

    tpfp = [tpfp_func(cls_dets[j], cls_gts[j], cls_gt_ignore[j], iou_thr, area_ranges) for j in range(len(cls_dets))]

    tp, fp = tuple(zip(*tpfp))
    
    for i in range(images_num):
       # print("the %d, %d, %d"%(i,tp[i],fp[i]))
       pass
    ...
    # for i in range(1):
        ...
        # print("cls_gts:", len(cls_gts))
        
        fntp = np.zeros(len(tp))
        for j, bbox in enumerate(cls_gts):
            # print("bbox:",bbox)
            # print(area_ranges)
            if area_ranges is None:
                num_gts[0] += np.sum(np.logical_not(cls_gt_ignore[j]))
                fntp[j] = np.sum(np.logical_not(cls_gt_ignore[j]))
            else:
        ...
        if len(fntp) == len(tp):
            ...
            for img_ind in range(len(tp)):
                c_num = 0
                for c in tp[img_ind][0]:
                    c_num += 1 if 1 == c else 0
                # if c_num != fntp[img_ind]:
                    # print(tp[img_ind], fntp[img_ind])
                    # pass
                # print(c_num, fntp[img_ind])
                per = (c_num / fntp[img_ind]) if fntp[img_ind] != 0 else 0
                #print(per)
                ap_c[i] += per
                    #print(tp[img_ind], fntp[img_ind], c_num)
        ...
        ap = new_average_precision(recalls, precisions, mode)
        eval_results.append({
            'num_gts': num_gts,
            'num_dets': num_dets,
            'recall': recalls,
    ...
    #for ind in range(len(ap_c)):
     #   print(ap_c[ind])
    core_num, coreless_num = nums
    ap_c[0] /= core_num
    ap_c[1] /= coreless_num
    print("AP_c:", ap_c)
    new_mAP = np.mean(ap_c)
    print("mAP:", new_mAP)
    ...
    if print_summary:
        new_print_map_summary(mean_ap, eval_results, dataset)

    return mean_ap, eval_results


def new_print_map_summary(mean_ap, results, dataset=None):
